...
export class ArticulosService {
 ...
 async SetSeleccionarArticuloDetalle(idArticulo: number, SiempreRecuperar: boolean) {
  ...
  SetFiltrarArticulos(filtros: SerializedFilterValues) {
    ...
    this.Articulosfiltrados = this.getArticulos().products;
  ...
  public async ConsultarDepartamento(IdEmpresa: number) {
    try {
      ...
      const config: any = await firstValueFrom(
        this.httpClient.get(this.UrlServicio + '/' + IdEmpresa.toString())
      );
  ...
  public async RecuperarArticulos(slug: string) {
    if (isPlatformBrowser(this.platformId)) {
      ...
      try {
        const response: ArticulosCarroComprasResponse = await firstValueFrom(this.httpClient.get<ArticulosCarroComprasResponse>(`${this.UrlServicio}/${IdEmpresa}/${TipoFiltro}/${Id}`));

        const { products } = response

        this.setFilters(products.filters);
        ...
        this.setArticulos$(response);

        this.setIsLoading(false);

      } catch (error) {
        this.setIsLoading(false);
        console.log('RecuperarArticulos Error al consumir servicio:' + error.message);
      }
